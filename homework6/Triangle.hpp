...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (dotProduct(ray.direction, normal) > 0)//normal is always pointing to the origin
        return inter;
    double u, v, t_tmp = 0;
    Vector3f pvec = crossProduct(ray.direction, e2); //pvec:s1
    double det = dotProduct(e1, pvec);//fenmu
    if (fabs(det) < EPSILON)
        return inter;
    ...
    if (v < 0 || u + v > 1)
        return inter;
    t_tmp = dotProduct(e2, qvec) * det_inv;//s2 x e2
